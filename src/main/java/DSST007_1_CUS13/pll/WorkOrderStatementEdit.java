 ...
 * @author praveen
 */
public class WorkOrderStatementEdit extends javax.swing.JInternalFrame {

    private static WorkOrderStatementEdit MyInstance;
    
    public WorkOrderStatementEdit() {
        initComponents();
    }
    ...
    private void initComponents()
    {
        ...
        setClosable(true);
        setTitle("View/Edit > Work Order Statement");
        ...
        gridBagConstraints.gridx = 0;
        gridBagConstraints.gridy = 0;
        jPanel3.add(print_jb, gridBagConstraints);

        update_jb.setBackground(new java.awt.Color(185, 197, 31));
        update_jb.setText("Save");
    ...
    public static WorkOrderStatementEdit getInstance() // Method
    {
        if (MyInstance == null)
	{
            MyInstance = new WorkOrderStatementEdit();
	}
        return MyInstance;
    }
    ...
    {//GEN-HEADEREND:event_update_jbMouseClicked
        if(update_jb.isEnabled() == true)
        {
            ...
            {
                updateWorkOrderStatement();
            } 
        }
    }//GEN-LAST:event_update_jbMouseClicked

    private boolean updateWorkOrderStatement()
    {
        boolean error = false;
        ...
        try
        {
            ...
            String work_order_no = ((JTextField)workOrderNos_jcb.getEditor().getEditorComponent()).getText();
            ...
            error = workOrderStatement.updateWorkOrderStatement(workOrderStatement);
    ...
    private boolean validationBeforeUpdate()
    {
        ...
        else if(amountRec_jtf.getText().equals(""))
        {
    ...
    {
        ((JTextField)workOrderNos_jcb.getEditor().getEditorComponent()).setText("");
        jobname_lbl.setText("--");
        customer_lbl.setText("--");
        jobdate_lbl.setText("--");
        rate_jtf.setText("");
...
//            }
    ...
    {//GEN-HEADEREND:event_rate_jtfKeyReleased
        netTotal_jtf.setText(totalAmount().toString());
    ...
    {
        String rate = rate_jtf.getText();
        BigDecimal rate_BD = null;
        String plate = plate_jtf.getText();
        BigDecimal plate_BD = null;
        String design = design_jtf.getText();
        BigDecimal design_BD = null;
        String quantity = quantity_jtf.getText();
        BigDecimal quantity_BD = null;
        String amount = amount_jtf.getText();
        BigDecimal amount_BD = null;
        String outStandingBal = outStandingBal_jtf.getText();
        BigDecimal outStandingBal_BD = null;
        String otherItem = otherItemAmt_jtf.getText();
        BigDecimal otherItem_BD = null;
        String cuttingCharges = cuttingCharges_jtf.getText();
        BigDecimal cuttingCharges_BD = null;
        BigDecimal total_BD = new BigDecimal("0.00");
        String deduction = deductionAmt_jtf.getText();
        BigDecimal deduction_BD = null;
        String netTotal = netTotal_jtf.getText();
        BigDecimal netTotal_BD = null;
    ...
    {
        if(!netTotal_jtf.getText().equals(""))
        {
            BigDecimal netTotal_bd = new BigDecimal(netTotal_jtf.getText());
            BigDecimal amtRecieved_bd = new BigDecimal(amountRec_jtf.getText());
            BigDecimal balAmt_bd = new BigDecimal("0.00");
            balAmt_bd = netTotal_bd.subtract(amtRecieved_bd);
            
            balAmount_jtf.setText(balAmt_bd.toString());
        }
        else
        {
            netTotal_jtf.setText("");
    ...
    private void load_workOrder_jcb()
    {
        ...
        {
            WorkOrderStatement workOrderStatement = new WorkOrderStatement();
            List<String> workOrderList = workOrderStatement.getWorkOrderNosFromWorkOrderStatements();
    ...
    private void displayWorkOrderDetails()
    {
        try
        {
            ...
            WorkOrderStatement workOrderStatement = new WorkOrderStatement();
            String workOrderId = ((JTextField)workOrderNos_jcb.getEditor().getEditorComponent()).getText();
            List<WorkOrderStatement> workOrderDetList = workOrderStatement.getWorkOrderDetailsFromWorkOrderStatements(workOrderId);
            for (int i = 0; i < workOrderDetList.size(); i++)
            {
                String customer = workOrderDetList.get(i).getCustomer();
                String jobname = workOrderDetList.get(i).getDescription();
                String jobDate = sdf.format(sdf_.parse(workOrderDetList.get(i).getJobdate()));
                String rate = workOrderDetList.get(i).getRate();
                String plate = workOrderDetList.get(i).getPlate();
                String design = workOrderDetList.get(i).getDesign();
                String quantity = workOrderDetList.get(i).getQuantity();
                String amount = workOrderDetList.get(i).getAmount();
                String outstanding_balance = workOrderDetList.get(i).getOutstanding_balance();
                String other_item = workOrderDetList.get(i).getOther_item();
                String other_item_amt = workOrderDetList.get(i).getOther_item_amt();
                String cutting_charges = workOrderDetList.get(i).getCutting_charges();
                String total = workOrderDetList.get(i).getTotal();
                String deduction = workOrderDetList.get(i).getDeduction();
                String deduction_amt = workOrderDetList.get(i).getDeduction_amt();
                String net_total = workOrderDetList.get(i).getNet_total();
                String amt_recieved = workOrderDetList.get(i).getAmt_recieved();
                String balance_amt = workOrderDetList.get(i).getBalance_amt();
                
                jobname_lbl.setText(jobname);
                customer_lbl.setText(customer);
                jobdate_lbl.setText(jobDate);
                rate_jtf.setText(rate);
                plate_jtf.setText(plate);
                design_jtf.setText(design);
                quantity_jtf.setText(quantity);
                amount_jtf.setText(amount);
                outStandingBal_jtf.setText(outstanding_balance);
                otherItem_jtf.setText(other_item);
                otherItemAmt_jtf.setText(other_item_amt);
                cuttingCharges_jtf.setText(cutting_charges);
                totalAmount_jtf.setText(total);
                deduction_jtf.setText(deduction);
                deductionAmt_jtf.setText(deduction_amt);
                netTotal_jtf.setText(net_total);
                amountRec_jtf.setText(amt_recieved);
                balAmount_jtf.setText(balance_amt);
            }
        }
        catch (Exception e)
        {
